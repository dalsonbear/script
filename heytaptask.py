...
# @Author  : hwkxk(丶大K丶)
# @Email   : k@hwkxk.cn
# 黑魔修改增加多账号签到支持 2021年7月16日09:53:11
# 配置看htconfig.ini
import requests,json,time,logging,traceback,os,random,notify2,datetime,configparser
...
def readConfig():
    try:
        ...
        logging.error('1.请检查是否在目录下建立了htconfig.ini')
...
def main(event, context):
    ...
    while x <= 9:
        ...
        if client != False:
            ...
            logger.info("账号[cookies"+str(x)+"]登录失败，换下一个账号")
            x += 1
            continue
        x += 1
    ...
    if users.has_option("dingding", 'dingtalkWebhook'):
        notify2.sendDing(users.get("dingding","dingtalkWebhook")) #钉钉推送日记          
    if users.has_option("telegramBot", 'tgToken'):
        notify2.sendTg(users.get("telegramBot","tgToken"),users.get("telegramBot","tgUserId")) #TG机器人推送日记
    if users.has_option("pushplus", 'pushplusToken'):
        notify2.sendPushplus(users.get("pushplus","pushplusToken")) #push+ 推送日记
    if users.has_option("enterpriseWechat", 'id'):
        notify2.sendWechat(users.get("enterpriseWechat","id"),users.get("enterpriseWechat","secret"),users.get("enterpriseWechat","agentld")) #企业微信通知
    if users.has_option("IFTTT", 'apiKey'):
        notify2.sendIFTTT(users.get("IFTTT","apiKey"),users.get("IFTTT","eventName")) #IFTTT 推送日记
    if users.has_option("Bark", 'Barkkey'):
        notify2.sendBark(users.get("Bark","Barkkey"),users.get("Bark","Barksave")) #Bark推送助手
